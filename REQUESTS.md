...
### 4-4. ArUco/ChArUco 検出描画時の4チャンネル画像対応（例外回避）
- OpenCV の ArUco 検出描画関数（`cv::aruco::drawDetectedMarkers` や `cv::aruco::drawDetectedCornersCharuco`）が 4 チャンネル画像をサポートしておらず、検出時にアサーション例外が発生する問題を解消すること。
- カメラキャプチャや PBO 転送のパフォーマンス向上のために 4 チャンネル（BGRA/RGB32）画像処理を維持しつつ、`Calibration` 内部 of 検出・描画処理のタイミングのみ一時的に 3 チャンネル（BGR）に変換（`cv::cvtColor`）して実行し、描画結果を元の 4 チャンネル画像に書き戻す設計とすること。
