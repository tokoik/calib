...
- **指示**: カメラ認識時やフォーマット一覧取得時（`updateFormatList`、`openDevice`）にデコーダが不要に初期化され、数秒のフリーズやエラーが発生する問題の解決。
- **対応**: リスト取得時には `setFormat` を行わず、ユーザーが「開始」ボタンを押した際（`capture.select`）にのみフォーマット適用・デコーダ初期化を実行する遅延評価（Lazy Initialization）を導入。UIについても動的生成の最適化や3分割ドロップダウン化を実施。

### 4. MFT バッファ管理と低遅延（Low Latency）化
- **指示**: キャプチャ画像に数秒（1〜2秒）の遅延（レイテンシ）が発生している問題の解決。
